...
import { skillUpdateSchema } from "@/lib/validations/resume-source";
import { prisma } from "@/lib/prisma";
import { verifySkillOwnership } from "@/lib/resume-source-helpers";

export const PUT = authenticatedHandler(async (request, { userId, params }) => {
  const existing = await verifySkillOwnership(params.id, userId);
  if (!existing) {
